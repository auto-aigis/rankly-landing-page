...
export default function OnboardingPage() {
  ...
  const handleNext = () => {
    ...
    if (step === 2 && !websiteUrl.trim()) {
      setError('Please enter your website URL');
      return;
    }
    if (step === 2 && !websiteUrl.match(/^https?:\/\/.+/)) {
  ...
  const handleSubmit = async () => {
    ...
    if (validCompetitors.length < 3) {
      setError('Please add at least 3 competitors');
      return;
    }
    setLoading(true);
    try {
      await coreApi.completeOnboarding(brandName, websiteUrl, validCompetitors);
      router.push('/dashboard');
    } catch (err) {
